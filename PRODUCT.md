...
## Overview
**Vision/Goal:** `design_skill` is a CLI tool designed to assist in generating, iterating, and polishing premium web frontends by codifying design principles and product context into structured markdown files.
...
**Key Dependencies:** No major external dependencies; relies heavily on the local filesystem to read/write context files (`PRODUCT.md`, `DESIGN.md`, `.design_skill/`).
...
1. **Command Routing:** User input via the `design_skill` CLI command is intercepted by `bin/cli.js`. The entry point parses arguments, validates them against the supported command list in `src/commands.js`, and routes execution to the respective module in `src/`.
2. **Context Initialization & Validation:** The `init` command uses Node's `readline` to prompt the user interactively, generating `PRODUCT.md` and `DESIGN.md` in the user's working directory. For all subsequent commands, `cli.js` intercepts the execution to ensure these context files exist and contain the `design_skill_initialized: true` flag. 
...
- **Initialization:** Before using advanced commands in any target directory, developers must run `design_skill init` to establish the foundational design context.
